...
// what you get. It is also the one path on the page that never renders JSON,
// so a wrong content type or a dropped filter is invisible on screen and
// lands in somebody's spreadsheet instead.
...
func TestFleetListsNameTheNode(t *testing.T) {
	...
	if err := protojson.Unmarshal(c.get("/search?q=proxy_pass").Body.Bytes(), &search); err != nil {
	...
	if !searchNodes["lb01"] || !searchNodes["lb02"] {
		t.Errorf("GET /search?q=proxy_pass does not name both nodes: %+v", searchNodes)
	}

	var snapshots pb.SnapshotsListResponse
	if err := protojson.Unmarshal(c.get("/snapshots").Body.Bytes(), &snapshots); err != nil {
		t.Fatalf("decode /snapshots: %v", err)
	}
	for _, host := range []string{"lb01", "lb02"} {
		...
		if !found {
			t.Errorf("GET /snapshots never names %s, so its two nginx rows are indistinguishable", host)
		}
	}

	var nodes pb.NodesListResponse
	if err := protojson.Unmarshal(c.get("/nodes").Body.Bytes(), &nodes); err != nil {
	...
	for _, host := range []string{"lb01", "lb02"} {
		...
		if !found {
			t.Errorf("GET /nodes never names %s, so its two nginx rows are indistinguishable", host)
		}
	}
}
