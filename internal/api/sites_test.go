...
func sitesFixture(t *testing.T) *authClient {
	t.Helper()
	db := testDB(t)
	...
	c.postJSON("/login", map[string]any{"username": "viewer", "password": "a good long password"})
	if c.cookie == "" {
...
func TestSitesListCarriesTheFleetsNumbers(t *testing.T) {
	...
	if err := json.Unmarshal(c.get("/sites?q=shop").Body.Bytes(), &filtered); err != nil {
	...
	if err := json.Unmarshal(c.get("/sites?q=nothing-serves-this").Body.Bytes(), &miss); err != nil {
		t.Fatalf("decode: %v", err)
	}
	if len(miss.Rows) != 0 {
		t.Error("/sites?q= with no match returned rows anyway")
	}

	// The selected-site panel reads Variants, and a ?site= naming a hostname
	// nothing serves has a selection and no variants — the API's job is just
	// not to 500 on an empty slice.
	w = c.get("/sites?site=nothing-serves-this")
	if w.Code != http.StatusOK {
		t.Errorf("GET /sites?site= naming an unserved hostname = %d, want 200", w.Code)
	}
}
...
func TestSiteDetailTabsCarryTheirOwnFacts(t *testing.T) {
	...
	w := c.get("/sites/shop.example.com")
	if w.Code != http.StatusOK {
		t.Fatalf("GET /sites/shop.example.com = %d\n%s", w.Code, w.Body.String())
	}
	var overview struct {
		Overview *struct {
			Routes []struct {
	...
	// seedNginx's server block has two location blocks (/api/ and /), so the
	// route table must carry both — not just the top-ranked one.
	if len(overview.Overview.Routes) != 2 {
		t.Errorf("overview routes = %+v, want both location blocks", overview.Overview.Routes)
	}
	foundRoute := false
	...
	if err := json.Unmarshal(c.get("/sites/shop.example.com?tab=nodes").Body.Bytes(), &nodesTab); err != nil {
	...
	if err := json.Unmarshal(c.get("/sites/shop.example.com?tab=upstreams").Body.Bytes(), &upstreamsTab); err != nil {
	...
	body := c.get("/sites/shop.example.com?tab=certificates").Body.Bytes()
	if err := json.Unmarshal(body, &certsTab); err != nil {
		t.Fatalf("decode certificates tab: %v", err)
	}

	// The variant filter keeps the site.
	if got := c.get("/sites/shop.example.com?variant=A").Code; got != http.StatusOK {
